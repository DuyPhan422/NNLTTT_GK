...
/**
 * Panel điểm danh dành cho Student.
 * Read-only hoàn toàn. Hiển thị lịch sử chuyên cần cá nhân.
 *
 * Layout:
 *  ┌─────────────────────────────────────────────────────┐
 *  │  KPI: Tổng buổi | Có mặt | Vắng | Trễ | Tỉ lệ %   │
 *  ├──────────────┬──────────────────────────────────────┤
 *  │ Sidebar:     │ Timeline theo lớp đang chọn          │
 *  │ Danh sách    │ [Ngày | Thứ | Trạng thái | Ghi chú]  │
 *  │ lớp đang học │                                      │
 *  └──────────────┴──────────────────────────────────────┘
 */
public class AttendanceStudentPanel extends JPanel {
    ...
    // grouped by classId → list of attendances
    ...
    public AttendanceStudentPanel(AttendanceService attendanceService,
        ...
        classSidebarList.setBackground(BG_SIDEBAR);
    ...
    private JPanel buildKpiRow() {
        ...
        row.add(buildKpiCard("📅 Tổng buổi",  kpiTotal,   BLUE));
        row.add(buildKpiCard("✅ Có mặt",      kpiPresent, GREEN));
        row.add(buildKpiCard("❌ Vắng mặt",    kpiAbsent,  RED));
        row.add(buildKpiCard("⏰ Đi trễ",      kpiLate,    AMBER));
        row.add(buildKpiCard("📊 Tỉ lệ có mặt", kpiRate,  BLUE));
        return row;
    }
    ...
    private JPanel buildClassSidebarItem(Long classId, String className,
        ...
        Color rateColor = rate >= 85 ? GREEN : rate >= 70 ? AMBER : RED;
    ...
    private JPanel buildProgressBar(double rate) {
        ...
        track.setBackground(new Color(226, 232, 240));
        track.setPreferredSize(new Dimension(0, 12));
        track.setBorder(BorderFactory.createEmptyBorder());

        Color fillColor = rate >= 85 ? GREEN : rate >= 70 ? AMBER : RED;
